...
class StickyForm extends Validation {
    ...
    public function validateForm($data, $formConfig) {
        foreach ($formConfig as $key => &$element) {
            ...
            // Handle name, email, password types
            if (isset($element['type']) && in_array($element['type'], ['name', 'email', 'password'])) {
                if (empty($element['value'])) {
                    $element['errorMsg'] = $customErrorMsg ?? 'This field is required.';
                    $formConfig['masterStatus']['error'] = true;
                } else {
                    $this->checkFormat($element['value'], $element['type'], $key, $customErrorMsg);
                    if (isset($this->getErrors()[$key])) {
                        $element['errorMsg'] = $this->getErrors()[$key];
                        $formConfig['masterStatus']['error'] = true;
                    }
                }
            }
